CLASS lhc_DrsJobConfig DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    "! Job template name — change after registering via zcl_drs_job_setup_catalog_and_template
...
CLASS lhc_DrsJobConfig IMPLEMENTATION.
  ...
  METHOD GET_INSTANCE_FEATURES.
    ...
    READ ENTITIES OF ZR_DRS_JOB_CONFIG IN LOCAL MODE
      ENTITY DrsJobConfig
  ...
  METHOD setDefaultValues.
    ...
    " Generate next JobId
    DATA LV_MAX_ID TYPE ZR_DRS_JOB_CONFIG-JobId.
    CLEAR LV_MAX_ID.
    SELECT SINGLE MAX( JobId )
      FROM ZR_DRS_JOB_CONFIG
      INTO @LV_MAX_ID.
    DATA LV_NEXT_ID TYPE ZR_DRS_JOB_CONFIG-JobId.
    CLEAR LV_NEXT_ID.
    ...
    MODIFY ENTITIES OF ZR_DRS_JOB_CONFIG IN LOCAL MODE
      ENTITY DrsJobConfig
      ...
      TRY.
          ...
          CASE LS_JOB-RunType.
            ...
            WHEN 'P'.
              ...
              " === MANUAL DATE SHIFT WORKAROUND FOR SAP timezone bugs ===
              " CL_APJ_RT_API wrapper missing IV_ADJUST_START_INFO natively.
              " When a user schedules at e.g., 5AM UTC+7 on Monday, it is 10PM UTC Sunday.
              " SAP strictly validates that the UTC start date (Sun) matches the Recurrence (Mon).
              " We manually shift the start timestamp forward to align with the first valid weekday.
              " IF LS_JOB-PeriodicGranularity = 'W' OR LS_JOB-PeriodicGranularity = 'WM'.
              ...
              "   CLEAR LV_SHIFT_DATE.
              "   CLEAR LV_SHIFT_TIME.
              "   " Convert the current target UTC timestamp back into UTC Date
              "   CONVERT TIME STAMP LS_START_INFO-TIMESTAMP TIME ZONE 'UTC'
              "     INTO DATE LV_SHIFT_DATE TIME LV_SHIFT_TIME.

              "   " Keep shifting the date forward +1 day at a time until it hits a valid Weekday
              "   DO 7 TIMES.
              "     DATA(LV_TEST_DOW) = CL_APJ_FW_UTILITIES=>COMPUTE_DAY( IV_DATE = LV_SHIFT_DATE ).
              "     DATA(LV_MATCH)    = ABAP_FALSE.
              "     CASE LV_TEST_DOW.
              "       WHEN 1. IF LS_JOB-OnMonday    = ABAP_TRUE. LV_MATCH = ABAP_TRUE. ENDIF.
              "       WHEN 2. IF LS_JOB-OnTuesday   = ABAP_TRUE. LV_MATCH = ABAP_TRUE. ENDIF.
              "       WHEN 3. IF LS_JOB-OnWednesday = ABAP_TRUE. LV_MATCH = ABAP_TRUE. ENDIF.
              "       WHEN 4. IF LS_JOB-OnThursday  = ABAP_TRUE. LV_MATCH = ABAP_TRUE. ENDIF.
              "       WHEN 5. IF LS_JOB-OnFriday    = ABAP_TRUE. LV_MATCH = ABAP_TRUE. ENDIF.
              "       WHEN 6. IF LS_JOB-OnSaturday  = ABAP_TRUE. LV_MATCH = ABAP_TRUE. ENDIF.
              "       WHEN 7. IF LS_JOB-OnSunday    = ABAP_TRUE. LV_MATCH = ABAP_TRUE. ENDIF.
              "     ENDCASE.

              "     IF LV_MATCH = ABAP_TRUE.
              "       " Found a matching day. Re-convert back to UTC timestamp for SAP.
              "       CONVERT DATE LV_SHIFT_DATE TIME LV_SHIFT_TIME
              "         INTO TIME STAMP LS_START_INFO-TIMESTAMP TIME ZONE 'UTC'.
              "       EXIT.
              "     ENDIF.
              "     " Shift +1 day forward
