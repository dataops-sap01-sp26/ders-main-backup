CLASS ZCL_XLSX_FORMATTER_AR02 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS ZCL_XLSX_FORMATTER_AR02 IMPLEMENTATION.
  ...
  METHOD ZIF_FILE_FORMATTER~GENERATE.
    ...
    LO_ZIP->ADD( NAME = 'xl/workbook.xml' CONTENT = LV_CONV->CONVERT(
      ...
      '<sheets>' &&
        '<sheet name="Customer Balances" sheetId="1" r:id="rId1"/>' &&
      '</sheets>' &&
      '</workbook>' ) ).
  ...
  METHOD BUILD_SHEET.
    ...
    LV_ROWS = LV_ROWS &&
      ...
      '<c r="A1" t="inlineStr" s="1">' &&
        '<is><t>Customer Account Balance Report</t></is>' &&
      '</c>' &&
      '</row>'.
