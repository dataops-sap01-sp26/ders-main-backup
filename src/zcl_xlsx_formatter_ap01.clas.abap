CLASS ZCL_XLSX_FORMATTER_AP01 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS ZCL_XLSX_FORMATTER_AP01 IMPLEMENTATION.
  ...
  METHOD ZIF_FILE_FORMATTER~GENERATE.
    ...
    LO_ZIP->ADD( NAME = 'xl/workbook.xml' CONTENT = LV_CONV->CONVERT(
      ...
      '<sheets>' &&
        '<sheet name="Supplier Open Items" sheetId="1" r:id="rId1"/>' &&
      '</sheets>' &&
      '</workbook>' ) ).
  ...
  METHOD BUILD_SHEET.
    ...
    LV_ROWS = LV_ROWS &&
      ...
      '<c r="A1" t="inlineStr" s="1">' &&
        '<is><t>SUPPLIER OPEN ITEMS REPORT</t></is>' &&
      '</c>' &&
      '</row>'.
    ...
    LV_ROWS = LV_ROWS &&
      ...
      '<c r="D7" t="inlineStr" s="3"><is><t>Open Payables</t></is></c>'     &&
      '<c r="E7" t="inlineStr" s="3"><is><t>Currency</t></is></c>'          &&
      '<c r="F7" t="inlineStr" s="3"><is><t>Overdue Days</t></is></c>'      &&
      '<c r="G7" t="inlineStr" s="3"><is><t>Due Date</t></is></c>'          &&
      '</row>'.
    ...
    DATA(LT_MAP) = VALUE TT_COL_MAP(
      ...
      ( FIELD = 'TOTALOPENAMOUNT'  IS_NUM = ABAP_TRUE  )
      ( FIELD = 'LOCALCURRENCY'    IS_NUM = ABAP_FALSE )
      ( FIELD = 'MAXDAYSOVERDUE'   IS_NUM = ABAP_TRUE  )
      ( FIELD = 'NETDUEDATE'       IS_NUM = ABAP_FALSE )
    ).
    ...
    LOOP AT <LT_DATA> ASSIGNING FIELD-SYMBOL(<LS_ROW>).
      ...
      LOOP AT LT_MAP INTO DATA(LS_MAP).
        ...
        IF SY-SUBRC = 0.
          ...
          ELSE.
            " Escape special chars (e.g. '&') to prevent Excel XML corruption
        ...
        ELSE.
          LV_TXT_STYLE = COND #( WHEN LV_COL_IDX = 5 THEN '8' ELSE '6' ).
    ...
    LV_ROWS = LV_ROWS &&
      ...
      |<c r="A{ LV_T2 }" t="inlineStr" s="9"><is><t>Total Open Payables:</t></is></c>| &&
      |<c r="B{ LV_T2 }" s="10"><f>SUM(D{ LV_DATA_FIRST }:D{ LV_DATA_LAST })</f></c>| &&
      '</row>'.

    LV_ROWS = LV_ROWS &&
      |<row r="{ LV_T3 }" ht="18" customHeight="1">| &&
      |<c r="A{ LV_T3 }" t="inlineStr" s="9"><is><t>Over Due Amount:</t></is></c>| &&
      |<c r="B{ LV_T3 }" s="10"><f>SUMIF(F{ LV_DATA_FIRST }:F{ LV_DATA_LAST },"&gt;0",D{ LV_DATA_FIRST }:D{ LV_DATA_LAST })</f></c>| &&
      '</row>'.
