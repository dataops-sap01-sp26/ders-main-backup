CLASS ZCL_REPORT_AR02 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS ZCL_REPORT_AR02 IMPLEMENTATION.
  ...
  METHOD ZIF_REPORT~EXECUTE.
    ...
    RS_RESULT-FILE_NAME_PREFIX = |Customer_Balances_{ MS_PARAMS-REPORT_ID }|.
  ENDMETHOD.


  METHOD QUERY_DATA.
    SELECT SINGLE * FROM ZDRS_PARAM_AR02
    ...
    " 2. Handle Customer Range (Prioritize specific range field, then From/To)
    DATA LT_CUSTOMER_RANGE TYPE RANGE OF KUNNR.
    IF LS_SPEC_PARAM-CUSTOMER_RANGE IS NOT INITIAL.
      APPEND VALUE #( SIGN = 'I' OPTION = 'EQ' LOW = LS_SPEC_PARAM-CUSTOMER_RANGE ) TO LT_CUSTOMER_RANGE.
    ELSEIF LS_SPEC_PARAM-CUSTOMER_FROM IS NOT INITIAL.
      APPEND VALUE #(
        SIGN   = 'I'
    ...
    " 3. Select Data using all available filters from the table definition
    SELECT FROM ZI_RPT_AR02
      FIELDS CompanyCode,
             Customer,
             CustomerName,
             Address,
      ...
      INTO TABLE @DATA(LT_AR02)
      UP TO @LV_MAX ROWS.

    CREATE DATA ER_DATA LIKE LT_AR02.
    FIELD-SYMBOLS <LT_OUT> TYPE STANDARD TABLE.
    ASSIGN ER_DATA->* TO <LT_OUT>.
    <LT_OUT> = LT_ar02.

    DATA(LO_STRUCT) = CAST CL_ABAP_STRUCTDESCR(
      CL_ABAP_TYPEDESCR=>DESCRIBE_BY_DATA( VALUE #( LT_ar02[ 1 ] OPTIONAL ) ) ).
