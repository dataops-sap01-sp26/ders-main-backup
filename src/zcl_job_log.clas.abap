    ...
    METHODS:
      LOG_JOB_START
        IMPORTING
          ...
          IV_JOB_CONFIG_UUID        TYPE ZDRS_JOB_CONFIG-JOB_UUID
          IV_REPORT_ID              TYPE ZDRS_SUBSCR-REPORT_ID
          IV_USER                   TYPE ZDRS_JOB_CONFIG-LAST_CHANGED_BY
        RETURNING
          VALUE(RV_JOB_HIST_UUID)   TYPE ZDRS_JOB_HISTORY-JOB_HIST_UUID,

      UPDATE_JOB_RESULTS
        IMPORTING
          IV_JOB_HIST_UUID TYPE ZDRS_JOB_HISTORY-JOB_HIST_UUID
          IV_FILE_UUID     TYPE ZDRS_FILE-FILE_UUID
          IV_OUTPUT_FORMAT TYPE ZDRS_SUBSCR-OUTPUT_FORMAT,

      LOG_JOB_END
        IMPORTING
          IV_JOB_HIST_UUID TYPE ZDRS_JOB_HISTORY-JOB_HIST_UUID
          IV_STEP_STATUS   TYPE ZDRS_JOB_HISTORY-JOB_STATUS OPTIONAL
          IV_JOB_STATUS    TYPE ZDRS_JOB_HISTORY-JOB_STATUS OPTIONAL
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      ...
      UPDATE_JOB_HISTORY
        IMPORTING
          IV_JOB_HIST_UUID TYPE ZDRS_JOB_HISTORY-JOB_HIST_UUID
          IV_STATUS        TYPE ZDRS_JOB_HISTORY-JOB_STATUS OPTIONAL
          IV_END_TIMESTAMP TYPE ZDRS_JOB_HISTORY-END_TIMESTAMP OPTIONAL
          IV_DURATION_MS   TYPE ZDRS_JOB_HISTORY-DURATION_MS OPTIONAL
          IV_FILE_UUID     TYPE ZDRS_FILE-FILE_UUID OPTIONAL
          IV_OUTPUT_FORMAT TYPE ZDRS_SUBSCR-OUTPUT_FORMAT OPTIONAL
...
CLASS ZCL_JOB_LOG IMPLEMENTATION.
  ...
  METHOD LOG_JOB_END.
    ...
    LV_FINAL_STATUS = COND #(
        WHEN IV_JOB_STATUS IS NOT INITIAL
        THEN IV_JOB_STATUS
        WHEN IV_STEP_STATUS IS NOT INITIAL
        THEN IV_STEP_STATUS
        ELSE 'F'
    ).
    ...
    UPDATE_JOB_HISTORY(
      ...
      IV_MESSAGE = COND STRING( WHEN IV_MESSAGE IS NOT INITIAL
                                THEN IV_MESSAGE )
    ).
  ENDMETHOD.
