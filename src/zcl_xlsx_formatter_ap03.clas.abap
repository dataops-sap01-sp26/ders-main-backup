CLASS ZCL_XLSX_FORMATTER_AP03 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS ZCL_XLSX_FORMATTER_AP03 IMPLEMENTATION.
  ...
  METHOD ZIF_FILE_FORMATTER~GENERATE.
    ...
    LO_ZIP->ADD( NAME = 'xl/workbook.xml' CONTENT = LV_CONV->CONVERT(
      ...
      '<sheets>' &&
        '<sheet name="AP Aging" sheetId="1" r:id="rId1"/>' &&
      '</sheets>' &&
      '</workbook>' ) ).
  ...
  METHOD BUILD_SHEET.
    ...
    LV_ROWS = LV_ROWS &&
      ...
      '<c r="A1" t="inlineStr" s="1">' &&
        '<is><t>ACCOUNT PAYABLE AGING REPORT</t></is>' &&
      '</c>' &&
      '</row>'.
    ...
    LV_ROWS = LV_ROWS &&
      ...
      '<c r="D7" t="inlineStr" s="3"><is><t>Total Payables</t></is></c>'       &&
      '<c r="E7" t="inlineStr" s="3"><is><t>Not Due</t></is></c>'              &&
      '<c r="F7" t="inlineStr" s="3"><is><t>0 - 30 Days</t></is></c>'          &&
      '<c r="G7" t="inlineStr" s="3"><is><t>31 - 60 Days</t></is></c>'         &&
      '<c r="H7" t="inlineStr" s="3"><is><t>61 - 90 Days</t></is></c>'         &&
      '<c r="I7" t="inlineStr" s="3"><is><t>&gt; 90 Days</t></is></c>'         &&
    ...
    LV_ROWS = LV_ROWS &&
      ...
      |<c r="A{ LV_T2 }" t="inlineStr" s="9"><is><t>Total Payables:</t></is></c>| &&
    ...
    LV_ROWS = LV_ROWS &&
      ...
      |<c r="B{ LV_T3 }" s="10">| &&
        |<f>SUM(F{ LV_DATA_FIRST }:F{ LV_DATA_LAST })| &&
           |+SUM(G{ LV_DATA_FIRST }:G{ LV_DATA_LAST })| &&
           |+SUM(H{ LV_DATA_FIRST }:H{ LV_DATA_LAST })| &&
           |+SUM(I{ LV_DATA_FIRST }:I{ LV_DATA_LAST })</f>| &&
      '</c>' &&
      '</row>'.
