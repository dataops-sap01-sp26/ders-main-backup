CLASS ZCL_XLSX_FORMATTER_AR01 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS ZCL_XLSX_FORMATTER_AR01 IMPLEMENTATION.
  ...
  METHOD ZIF_FILE_FORMATTER~GENERATE.
    ...
    LO_ZIP->ADD( NAME = 'xl/workbook.xml' CONTENT = LV_CONV->CONVERT(
      ...
      '<sheets>' &&
        '<sheet name="Open Items" sheetId="1" r:id="rId1"/>' &&
      '</sheets>' &&
      '</workbook>' ) ).
  ...
  METHOD BUILD_SHEET.
    ...
    LV_ROWS = LV_ROWS &&
      ...
      '<c r="A1" t="inlineStr" s="1">' &&
        '<is><t>CUSTOMER OPEN ITEMS REPORT</t></is>' &&
      '</c>' &&
      '</row>'.
    ...
    LV_ROWS = LV_ROWS &&
      ...
      '<c r="D7" t="inlineStr" s="3"><is><t>Open Receivables</t></is></c>'    &&
    ...
    LV_ROWS = LV_ROWS &&
      ...
      |<c r="A{ LV_T2 }" t="inlineStr" s="9"><is><t>Total Open Amount:</t></is></c>| &&
    ...
    LV_ROWS = LV_ROWS &&
      ...
      |<c r="B{ LV_T3 }" s="10">| &&
        |<f>SUMIF(F{ LV_DATA_FIRST }:F{ LV_DATA_LAST },"&gt;0",D{ LV_DATA_FIRST }:D{ LV_DATA_LAST })</f>| &&
      '</c>' &&
      '</row>'.
