...
define root view entity ZI_RPT_GL_ACCOUNT_BALANCES
  -- Bảng nguồn phải là Journal Entry Item (Dữ liệu kế toán)
  as select from    I_JournalEntryItem      as J 
    
//    -- Join lấy Tên tài khoản
//    inner join      I_GLAccountText         as T on  T.GLAccount    = J.GLAccount
//                                            and T.Language     = $session.system_language
//                                            
//    -- Join lấy Fiscal Year Variant từ Company Code để lọc động
//    inner join      I_CompanyCode           as C on  C.CompanyCode = J.CompanyCode
//    
//    -- Join với bảng Kỳ để xác thực (giúp loại bỏ các Variant rác)
//    inner join      I_FiscalYearPeriod      as P on  P.FiscalYearVariant = C.FiscalYearVariant
//                                            and P.FiscalYear        = J.FiscalYear
//                                            and P.FiscalPeriod      = J.FiscalPeriod
    left outer join I_GLAccountText  as T on  T.GLAccount    = J.GLAccount
                                        and T.Language     = $session.system_language
                                        and T.ChartOfAccounts = J.ChartOfAccounts
                                                                                
    composition [0..*] of ZI_RPT_GL_ACCOUNT_BALANCES_I as _Items
